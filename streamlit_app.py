...
def compute_funnel(df: pd.DataFrame, cohort_ids: set) -> dict:
    ...
    return {
        ...
        "s_login": s_login,
        "s_create_assess": s_create_assess,
        "s_grade": s_grade,
        "s_refine": s_refine,
        "activated": activated,
        "graded_after_activation": graded_after_activation,
        "refined_after_grading": refined_after_grading
    }
...
def compute_trust_ratio(df: pd.DataFrame, window_minutes=5):
    ...
    for uid, g in grp:
        ...
        for i, r in rows.iterrows():
            ...
            tmax = t0 + timedelta(minutes=window_minutes)
...
def centred_funnel_chart(funnel_counts: dict):
    ...
    # Normalise for width display
    max_v = max(values) if values else 1
    widths = [v/max_v for v in values]
    # Plot centered horizontal bars
    y = np.arange(len(steps))
    # Center by shifting bars around 0 using symmetric widths
    lefts = [-w/2 for w in widths]
    ax.barh(y, widths, left=lefts)
    ax.set_yticks(y, steps)
    ax.set_xlim(-1, 1)
    # Label counts on the right
    for i, (v, w) in enumerate(zip(values, widths)):
        ax.text(0.55, i, f"{v}", va="center")
    ax.set_xticks([])
    ax.set_xlabel("Centered relative width")
    st.pyplot(fig)
...
funnel_counts, funnel_sets = compute_funnel(df_logs, cohort_ids)
st.subheader("Funnel (by teacher)")
centred_funnel_chart(funnel_counts)
...
ax2.plot(daily_usage["date_sgt"], daily_usage["logins"], marker="o")
ax2.set_xlabel("Date (SGT)")
ax2.set_ylabel("Logins")
st.pyplot(fig2)

fig3, ax3 = plt.subplots(figsize=(8,3))
ax3.plot(daily_usage["date_sgt"], daily_usage["grading_events"], marker="o")
ax3.set_xlabel("Date (SGT)")
ax3.set_ylabel("Grading events")
st.pyplot(fig3)
