...
# Onboarding sessions (Singapore time, UTC+8)
SCHOOL_ONBOARDING = {
    "ACSI": {"date": "2025-08-12", "start": "14:30", "end": "15:30"},  # 2:30-3:30pm
    "Anglican": {"date": "2025-08-12", "start": "16:00", "end": "17:00"},  # 4-5pm 
    "Ngee Ann": {"date": "2025-08-13", "start": "15:30", "end": "16:30"},  # 3:30-4:30pm
    "Bartley": {"date": "2025-08-13", "start": "15:30", "end": "16:30"},  # 3:30-4:30pm
    "Northlight": {"date": "2025-08-18", "start": "15:00", "end": "16:00"},  # 3-4pm
    "St Andrew": {"date": "2025-08-20", "start": "10:30", "end": "11:30"},  # 10:30-11:30am
    "Pei Hwa": {"date": "2025-08-20", "start": "15:30", "end": "16:30"},  # 3:30-4:30pm
}
...
st.sidebar.markdown("---")
exclude_onboarding_for_funnel = st.sidebar.checkbox(
    "Exclude onboarding day from funnel metrics", value=True
)
...
# Check if timestamp falls within school's onboarding session
def is_onboarding_time(row):
    """Check if timestamp falls within school's onboarding session"""
    school = row["school"]
    if school not in SCHOOL_ONBOARDING:
        return False
    
    session = SCHOOL_ONBOARDING[school]
    session_date = pd.to_datetime(session["date"])
    start_time = pd.to_datetime(f"{session['date']} {session['start']}")
    end_time = pd.to_datetime(f"{session['date']} {session['end']}")
    
    # Convert UTC timestamp to Singapore time (+8)
    sg_time = row["@timestamp"] + pd.Timedelta(hours=8)
    
    return (sg_time >= start_time) & (sg_time <= end_time)

# Filter out onboarding sessions
logs["is_onboarding"] = logs.apply(is_onboarding_time, axis=1)
logs = logs[~logs["is_onboarding"]]

st.title("Mark.ly Pilot Dashboard")

# ===== Adoption Funnel =====
st.header("Adoption Funnel" + (" (excluding onboarding)" if exclude_onboarding_for_funnel else ""))

# Filter logs for funnel if excluding onboarding
funnel_logs = logs[logs["date_local"] > logs["first_login_date"]].copy() if exclude_onboarding_for_funnel else logs.copy()

emails_df = pd.DataFrame({"email": funnel_logs["email"].unique()})
email_school_map = funnel_logs[["email", "school"]].drop_duplicates().set_index("email")["school"]
